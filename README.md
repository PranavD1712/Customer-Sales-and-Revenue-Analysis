##### **Customer Sales \& Revenue Analytics**



###### **Project Overview:**
This project performs an end-to-end analysis of sales, profit, customer behavior, and regional performance using the Superstore dataset. The objective is to uncover profitability drivers, identify loss-making segments, analyze customer patterns, and present insights through an interactive Power BI dashboard.



###### **Tools \& Technologies:**

Python (Pandas, NumPy, Matplotlib, Seaborn)
Google Colab Notebook
Power BI Desktop
Git \& GitHub



###### **Project Requirements:**

**System Requirements**
Operating System: Windows / macOS / Linux
Python Version: 3.8 or above
Power BI Desktop (latest version recommended)



**Python Libraries Used**
pandas
numpy
matplotlib
seaborn



**Dataset**
Sample – Superstore Dataset(From Kaggle)
Format: CSV
Contains order-level data on sales, profit, customers, products, discounts, and regions





###### **Project Structure:**

Sales-Profit-Analysis/
│
├── data/
│   └── Sample - Superstore.csv
│
├── notebook/
│   └── Customer\_Sales\_\&\_Revenue\_Analysis.ipynb
│
├── powerbi/
│   └── Customer\_Sales\_\&\_Revenue\_Analysis\_PowerBI.pbix
│
├── visuals/
│   └── dashboard\_overview.png
│
├── README.md





###### **Project Phases:**
**Data Understanding** – Dataset structure, variables
**Data Cleaning** – Handling missing values, duplicates, and data quality issues
**Feature Engineering** – Profit margin, profit flags, customer frequency
**Exploratory Data Analysis (EDA)** – Sales, profit, discount, customer, and regional analysis
**Business Insights \& Recommendations** – Translating analysis into actionable findings
**Power BI Dashboard** – Interactive visual storytelling and KPI tracking



**Power BI Dashboard:**
KPI Cards: Total Sales, Total Profit, Profit Margin, Loss Orders
Sales \& Profit Trends
Category \& Sub-Category Performance
Regional Profitability Analysis
Customer Contribution \& Frequency Analysis



**Key Business Insights:**
* High sales do not always translate into high profit.
  Technology category generates the highest profit.
* Furniture category is frequently loss-making.
* Profit margins decline sharply at higher discount levels.
* Majority of customers are one-time buyers.
* A small group of customers contributes disproportionately to revenue.





###### **Business Value:**
This analysis helps organizations:
Identify loss-making products and regions
Optimize discount strategies
Focus on high-value customers
Improve profitability through data-driven decisions



**Author**
Pranav Deshmukh
Data Analyst | Python | SQL | Power BI



**Notes**:
Analysis is documented in a single, well-structured Google Colab Notebook
Power BI .pbix file is included for interactive exploration
Dashboard screenshots are provided for quick preview without Power BI


